    ...
    if (config.sendEmails && reports.length > 0) {
      ...
      const results = await sendBatchEmails(
        ...
        (reportData: Record<string, unknown>) => ({
          to: reportData.email as string,
          subject: `Your Weekly Yield Report - ${(reportData.period as Record<string, unknown>).startDate} to ${(reportData.period as Record<string, unknown>).endDate}`,
